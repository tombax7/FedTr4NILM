...
class Trainer:
    def __init__(self,args,ds_parser,model): 
        ...
        self.single_client       = ds_parser.single_client
    ...
    def train(self, args):
        ...
        for epoch in range(self.num_epochs):
            ...
            mae, mre, acc, precision, recall, f1 = self.train_validate()
            
            self.update_arguments_metrics(mae, mre, acc, precision, recall, f1, args, 'train')

        mae, mre, acc, precision, recall, f1 = self.validate()
        
        self.update_arguments_metrics(mae, mre, acc, precision, recall, f1, args,'val')
    ...
    def train_one_epoch(self,epoch):
        ...
        for _,batch in enumerate(tqdm_dataloader):
            ...
            average_loss = np.mean(np.array(loss_values))
            self.training_loss.append(average_loss)
            tqdm_dataloader.set_description(' We train client {}, for inner epoch {}, loss {:.2f}'.format(self.single_client, epoch, average_loss))
    ...
    def validate(self ):
        ...
        with torch.no_grad():
            ...
            for _,batch in enumerate(tqdm_dataloader):
                ...
                self.update_metrics_dict(mae,mre,acc,precision,recall,f1, 'val')
                ...
                tqdm_dataloader.set_description('Validation, rel_err {:.2f}, acc {:.2f}, f1 {:.2f} , loss {:.2f}'.format(mre_mean, acc_mean, f1_mean, average_loss))
    ...
    def train_validate(self ):
        ...
        with torch.no_grad():
            ...
            for _,batch in enumerate(tqdm_dataloader):
                ...
                self.update_metrics_dict(mae,mre,acc,precision,recall,f1, 'train')
            
                
                loss_values.append(total_loss.item())
                average_loss = np.mean(np.array(loss_values))
                self.trainval_loss.append(average_loss)


                
                acc_mean     = np.mean(np.concatenate(self.train_metrics_dict['acc']).reshape(-1))
                f1_mean      = np.mean(np.concatenate(self.train_metrics_dict['f1'] ).reshape(-1))
                mre_mean     = np.mean(np.concatenate(self.train_metrics_dict['mre']).reshape(-1))
                tqdm_dataloader.set_description('Update train metrics, rel_err {:.2f}, acc {:.2f}, f1 {:.2f} , loss {:.2f}'.format(mre_mean, acc_mean, f1_mean, average_loss))
        
        return [np.array(np.concatenate(v)).mean(axis=0) for v in self.train_metrics_dict.values()]
    ...
    def test(self,test_loader):
        ...
        self._save_result({'gt': self.y_curve.tolist(),'pred': self.y_pred_curve.tolist()}, 'test_result'+self.single_client+'.json')
        mae, mre = regression_errors(self.y_pred_curve, self.y_curve)
        acc, precision,recall, f1 = acc_precision_recall_f1_score(self.s_pred_curve, self.status_curve)

        return mre,mae,acc, precision,recall, f1,average_loss
    ...
    def update_metrics_dict(self,mae,mre,acc,precision,recall,f1, mode): 
        if mode=='train':
    ...
    def update_arguments_metrics(self, mae, mre , acc ,precision, recall ,f1,  args, mode):  #mode = 'val
        if mode=='train':
            args.train_metrics[self.single_client]['mae'      ].append(mae)
            args.train_metrics[self.single_client]['mre'      ].append(mre)
            args.train_metrics[self.single_client]['acc'      ].append(acc)
            args.train_metrics[self.single_client]['precision'].append(precision)
            args.train_metrics[self.single_client]['recall'   ].append(recall)
            args.train_metrics[self.single_client]['f1'       ].append(f1)
            
            
            
        elif mode=='val':
            args.val_metrics[self.single_client]['mae'      ].append(mae)
            args.val_metrics[self.single_client]['mre'      ].append(mre)
            args.val_metrics[self.single_client]['acc'      ].append(acc)
            args.val_metrics[self.single_client]['precision'].append(precision)
            args.val_metrics[self.single_client]['recall'   ].append(recall)
            args.val_metrics[self.single_client]['f1'       ].append(f1)
           
        else: 
            args.test_metrics[self.single_client]['mae'      ].append(mae)
            args.test_metrics[self.single_client]['mre'      ].append(mre)
            args.test_metrics[self.single_client]['acc'      ].append(acc)
            args.test_metrics[self.single_client]['precision'].append(precision)
            args.test_metrics[self.single_client]['recall'   ].append(recall)
            args.test_metrics[self.single_client]['f1'       ].append(f1) 
