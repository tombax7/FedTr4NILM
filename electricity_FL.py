...
if __name__ == "__main__":
    ...
    for communication_round in range(args.max_communication_rounds):   
        ...
        for single_client in args.dis_cvs_files:
            ...
            if args.num_epochs > 0:
                ...
                model_all[single_client].to('cpu')
                
        average_model(args, model_avg, model_all)   #average model for every communication round (train for e inner epochs for the clients selected)#model1 model2 
        model_avg.to('cpu')

    
    #Saving final model_avg#
    if not os.path.exists(results_root):
            os.makedirs(results_root)
    print('Saving best model...')
    torch.save(model_avg.state_dict(), results_root.joinpath('best_acc_model.pth'))
    
    
    model_avg.load_state_dict(torch.load(results_root.joinpath('best_acc_model.pth')))
    
    model_all = Partial_Client_Selection(args, model_avg)
    ...
    with open(results_root.joinpath("train_metrics_fed.json"), "w") as file:
        json.dump(args.train_metrics, file)
    
    with open(results_root.joinpath("val_metrics_fed.json"), "w") as file:
        json.dump(args.val_metrics, file)
    
    with open(results_root.joinpath("test_metrics_fed.json"), "w") as file:
